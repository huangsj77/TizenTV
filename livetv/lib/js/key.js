...
function VatataKeyMoveClass() {
  ...
  this.setFocus42 = function () {
    ...
    this.doFocus = function (ele) {
      ...
      for (var i = 0; i < eles.length; i++) {
        ...
        try {
          ...
          console.log("focus 42 , get onblur func exception: " + e)
        }
      }
      m_currentEle = ele;
      ...
      console.log("m_currentEle scrollHeight = " + m_currentEle.parentNode.scrollHeight);
      console.log("m_currentEle.offsetTop = " + m_currentEle.offsetTop)
      console.log("parentNode.offsetHeight = "+ m_currentEle.parentNode.offsetHeight)
      console.log("parentNode.scrollTop = "+ m_currentEle.parentNode.scrollTop)
      console.log("postion = " +this.position);
      
      if( this.position == "down") {
    	  if( (m_currentEle.offsetTop+80) > (m_currentEle.parentNode.offsetHeight + m_currentEle.parentNode.scrollTop)) {
    		  	console.log("move down");
    	         m_currentEle.parentNode.scrollTop += 100;
    	  }
      } else if(this.position == "up"){
    	  if( m_currentEle.offsetTop >0 && 
	    		 (m_currentEle.offsetTop -100) < ( m_currentEle.parentNode.scrollTop ) ) {
    		  console.log("move up");
	        m_currentEle.parentNode.scrollTop -= 100;
	      }	
      }
      ...
      try {
        ...
        console.log("focus 42 , get onfocus func exception: " + e + " : " + ele)
      }
    }
    ...
    p.onblur = function () {
      p.focus();
      setTimeout(function () { p.focus(); }, 500);
    }
    p.focus();
  ...
  this.vttkeymove = function (e) {
    ...
    this.position  = "";
    switch (e.keyCode) {
      case KEY_W: { this.position = "up"; m_currentEle = _VttPos.findUp(m_currentEle); } break;
      case KEY_A: { this.position = "left"; m_currentEle = _VttPos.findLeft(m_currentEle); } break;
      case KEY_S: { this.position = "down"; m_currentEle = _VttPos.findDown(m_currentEle); } break;
      case KEY_D: { this.position = "right"; m_currentEle = _VttPos.findRight(m_currentEle); } break;
      case KEY_F: {
        this.doEnter(m_currentEle,e.keyCode);
      } break;
      case KEY_B: {
    	  var liveList = document.getElementById("list-controls");
    	  if(!liveList.classList.contains("list-controls-hide")) {
    		  liveList.classList.add("list-controls-hide");
    	  } else {
        	  player.stop();
        	  player=null; 
        	  window.history.back();
