  ...
  this.setFocus42 = function()  {
    ...
    this.doFocus = function( ele ) {
      ...
      for(var i=0; i<eles.length; i++ ) {
        var e = eles[i];
        // console.log("dofocus  blur from: " + e.outerHTML )
        e.className = e.className.replace( this.focusClassReg, "" );
        try{
          if ( !! e.onblur ) {
            var f = e.onblur.bind(e);
            f();            
          }
          // var bfunc = e._onblur_func ;
          // if ( ! bfunc ) {
          ...
          //     bfunc = function() {}
          //   }
          //   e._onblur_func = bfunc;
          // }
          // console.log("try to run blur ... " + bfunc);
          // bfunc();
        }catch(e){
      ...
      m_currentEle = ele;
      // console.log("dofocus to: " + ele.outerHTML )
      ele.className+=(" "+ this.focusClass );
      try {
        var bfunc = ele._onfocus_func ;
        if ( ! bfunc ) {
...
//    const KEY_ENTER = 66;
  ...
  this.vttkeymove = function(e){
    ...
    // console.log("old ele: " + m_currentEle.offsetWidth + " : " + m_currentEle.outerHTML )	
    if ( ! m_currentEle || m_currentEle.offsetWidth  == 0 ) {
      switch(e.keyCode){
        case KEY_W:{m_currentEle = document.body['up']( m_currentEle );}break;
        case KEY_A:{m_currentEle = document.body['left']( m_currentEle );}break;
        case KEY_S:{m_currentEle = document.body['down']( m_currentEle);}break;
        case KEY_D:{m_currentEle = document.body['right']( m_currentEle );}break;
      }
    ...
    // console.log("new ele: " + m_currentEle.offsetWidth + " : " + m_currentEle.outerHTML ) 
		if(m_currentEle != o_ele ) {
      this.doBlur( o_ele );
      this.doFocus(m_currentEle);
    }
    ...
    this.focusCurEle = function() {
        if ( m_currentEle != null && m_currentEle.offsetWidth > 0 ) {
          this.doFocus(m_currentEle);           
        }
    }

    this.initByIdIfChanged = function(id){
        //先清除原来的高亮元素,再设置focus元素
        if ( m_currentEle != null && m_currentEle.offsetWidth > 0 ) {
          this.doFocus( m_currentEle );
          return;
