	...
	this.load = function( cid ) {
		...
		uget.onsuccess = function(u,d) {
			...
			try {
				...
				console.log("catelist get e: " + e + " : " + _this.apiurl );
			}
			_this.category = _ugetdata.category;
			_this.info = _ugetdata.content;
			_this.onload(_this.category,_this.info);
		}
...
function ItemList( ) {
	...
	this.load = function( cid , pageNo, pageSize ) {
		...
		// console.log("itemlist load " + this.apiurl );
		var _this = this;
		var uget = new EasyGet();
		uget.onsuccess = function(u,d) {
			var _ugetdata ;
			try {
				...
				console.log("itemlist get e: " + e + " : " + _this.apiurl );
			}
			// eval( '_ugetdata  = ' + d );
	...
	this.load = function( mid ) {
		...
		uget.onsuccess = function(u,d) {
			...
			try {
				...
				console.log("mediaitem get e: " + e + " : " + _this.apiurl );
			}
			// eval( '_ugetdata = ' + d );
...
function MessageList() {
    this.skipcache = true;
    this.load = function( rid, name, room, msg, other ) {
        var url = PUSH_URL.replace("{RID}" , rid)
            .replace("{NAME}", encodeURIComponent(name) )
            .replace("{ROOM}", encodeURIComponent(room) )
            .replace("{MSG}",  encodeURIComponent(msg)  )
        ...
        console.log("push_message=" +url);
        var _this = this;
        var uget = new EasyGet();
        uget.onsuccess = function(u,d) {
            _this.megInfo = d;
            _this.onload(_this.megInfo);
        }
        uget.open( url ,this.skipcache );
    }
    this.onload = function() {};
}
