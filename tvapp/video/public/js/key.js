   ...
   findNearest : function( element , func )
   {
       ...
       do {
          ...
          try {
              ...
              // console.log("check " + d + " in " + parentEle.outerHTML )
              if ( !! parentEle[d] ) {
                var ele = parentEle[d]();
                //if ( ele != null ) 
                return ele;
  ...
  this.setFocus42 = function()  {
    ...
    this.doFocus = function( ele ) {
      ...
      for(var i=0; i<eles.length; i++ ) {
        var e = eles[i];
        e.className = e.className.replace( this.focusClassReg, "" ).replace( /\s+/g, " ");
        try{
          if ( ! e.blurfunc ) {
            e.blurfunc = (function() {
              var bf = this.getAttribute("onblur");
              eval( bf );              
            }).bind(e);
          }
          e.blurfunc();
        }catch(ex){
          console.log("focus 42 , get onblur func exception: " + ex)
        }
      }
      m_currentEle = ele;
      ...
      try {
        var e = ele;
        if ( ! e.focusfunc ) {
          e.focusfunc = ( function() {
            var bf = this.getAttribute("onfocus");
            eval( bf );            
          }).bind(e);
        }
        e.focusfunc();
      }catch(ex){ 
        console.log("focus 42 , get onfocus func exception: " + ex + " : " + ele )
      }
    }
